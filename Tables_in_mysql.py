...
db = cntr.connect(host = 'localhost' , user = 'root' , passwd = 'glhf123')
db.autocommit = True
cur = db.cursor()
cur.execute('drop database shop_management')
cur.execute("create database if not exists shop_management")
cur.execute("use shop_management")
cur.execute("create table stock(Book_No bigint primary key,Book_Name varchar(255),Author varchar(255),Publisher varchar(255),Cost_per_Book float,Available_Stock bigint,qty_purchased bigint,purchased_on date)")
